...
function log.toboolean(str)
  if type(str) ~= "string" or (str ~= "true" and str ~= "false") or not str
  then
    return nil
  end
  return str == "true"
...
do
  ...
  log[x.name] = function(message, status)
    if status ~= nil and type(status) ~= 'boolean'
    then
    ...
    -- if Yes, then print
    if (type(x.status) == 'boolean' and x.status == false) or x.status == nil
    then
      return
    end
